...
template <class T> static void CreateLabels(const i3d::Histogram &hist, std::vector<std::pair<T, size_t> > &lab, size_t tpos)
{
	...
	if (lab.size() == 0)
	{
		printf("Empty frame! (T=%d)\n", static_cast<int>(tpos));
		exit(-1);
	}
}
...
{
	return level.m_res_match[FindLabel(level.m_res_lab, label)];
}
...
int main(int argc, char **argv)
{
	if (argc != 10)
	{
		printf("A simple console application intended for calculating the AOGM measure\n"
			   "Copyright (C) 2018 Centre for Biomedical Image Analysis, Masaryk University\n\n"
			   "Usage: %s dir_gt dir_res num_digits w_ns w_fn w_fp w_ed w_ea w_ec\n", argv[0]);
		 return -1;
	}

	wxString gt_pref = wxString::Format("%s%c", argv[1], wxFileName::GetPathSeparator());
	wxString res_pref = wxString::Format("%s%c", argv[2], wxFileName::GetPathSeparator());
	...
	if (wxDir::Exists(gt_pref))
	{
		if (wxDir::Exists(res_pref))
		{
			...
			int tmp_digits = atoi(argv[3]);

			if (tmp_digits > 1)
			{
				num_digits = static_cast<size_t>(tmp_digits);
				res_fname_temp = wxString::Format("%smask%%0%dd.tif", res_pref, num_digits);
			}
			else
			{
				printf("The number of digits used for encoding temporal positions must be at least 2! (num_digits=%d)\n", tmp_digits);
				return -1;
			}

			PenaltyConfig penalty(atof(argv[4]), atof(argv[5]), atof(argv[6]), atof(argv[7]), atof(argv[8]), atof(argv[9]));
			double aogm = 0.0;
			...
			for (size_t i = 0; i < files.size(); ++i)
			{
				...
				{
					ClassifyLabels(static_cast<const char *>(files[i].c_str()), static_cast<const char *>(wxString::Format(res_fname_temp, tpos).c_str()), levels, penalty, aogm, max_split, log, min_tpos);
				}
			}
			...
			else
			{
				...
				// display the AOGM value
				printf("AOGM value: %g\n", aogm);

				// create the log file
				wxFile log_file;
				wxString log_name = res_pref + "AOGM_log.txt";
				...
				if (log_file.Create(log_name, true))
				{
					...
					log_file.Write(wxString::Format("AOGM value: %g", aogm) + wxTextFile::GetEOL());
				}
				else
				{
